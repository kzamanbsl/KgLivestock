...
namespace Firm.Service.Services.FeedEntry_Services
{
    public class FeedEntryService : IFeedEntryService
    {
        ...
        public async Task<FeedEntryServiceVM> AddNewFeedEntry(FeedEntryServiceVM model)
        {
            ...
            //        //for existing category in stock
            //        bool isExists = await context.FeedCurrentStocks.AnyAsync(c => c.FeedCategoryId == model.FeedCategoryId);
            //        if (isExists)
            //        {
            //            var feedExCurrentStocks = await context.FeedCurrentStocks.FirstOrDefaultAsync(c => c.FeedCategoryId == model.FeedCategoryId);
            //            feedExCurrentStocks.CurrentQuantity = feedExCurrentStocks.CurrentQuantity + feedEntry.Quantity;
            //            feedExCurrentStocks.CurrentUnitPrice = (feedExCurrentStocks.CurrentUnitPrice + feedEntry.UnitPrice) / 2 ;
            //            feedExCurrentStocks.UpdatedOn = feedEntry.CreatedOn;
            //            context.Entry(feedExCurrentStocks).State = EntityState.Modified;
        ...
        public async Task<List<FeedEntryServiceVM>> GetAll()
        {
            ...
            var data = await context.FeedEntries.Where(x => x.IsActive).ToListAsync();
        ...
        {
            //bool isExists = await context.FeedEntries.AnyAsync(c => c.FeedCategoryName == model.FeedCategoryName && c.Id != model.Id);
            //if (isExists)
            //{
            //    model.ErrorMessage = "FeedEntry already exists. Please add another Phone Number.";
            //    return model;
            //}
            ...
            //    if (feedEntry != null)
            //    {
            //        decimal quantity = feedEntry.Quantity;
            //        decimal unitPrice = feedEntry.UnitPrice;
            //        feedEntry.Date = model.Date;
            //        feedEntry.FeedCategoryId = model.FeedCategoryId;
            //        feedEntry.Quantity = model.Quantity;
            //        feedEntry.UnitPrice = model.UnitPrice;
            ...
            //            {
            //                var feedExCurrentStocks = await context.FeedCurrentStocks.FirstOrDefaultAsync(c => c.FeedCategoryId == model.FeedCategoryId);
            //                feedExCurrentStocks.CurrentQuantity = (feedExCurrentStocks.CurrentQuantity - quantity) + model.Quantity;
            //                //get the previous state of unit price
            //                feedExCurrentStocks.CurrentUnitPrice = (feedExCurrentStocks.CurrentUnitPrice *2) - unitPrice;
            //                //update the unit price again with new
            //                feedExCurrentStocks.CurrentUnitPrice = (feedExCurrentStocks.CurrentUnitPrice + feedEntry.UnitPrice) / 2;
            //                feedExCurrentStocks.UpdatedOn = feedEntry.CreatedOn;
            //                context.Entry(feedExCurrentStocks).State = EntityState.Modified;
            //                await context.SaveChangesAsync();
                ...
                if (feedEntry != null)
                {
                    ...
                    if (res != 0)
                    {
                        ...
                        if (isExists)
                        {
                            ...
                            // Update the current quantity and unit price
                            // feedCurrentStock.CurrentUnitPrice = (feedCurrentStock.CurrentUnitPrice * feedCurrentStock.CurrentQuantity) - (originalUnitPrice * originalQuantity) + (feedEntry.UnitPrice * feedEntry.Quantity) / (feedCurrentStock.CurrentQuantity + feedEntry.Quantity);
